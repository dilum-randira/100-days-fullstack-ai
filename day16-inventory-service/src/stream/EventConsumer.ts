...
export class EventConsumer {
  ...
  constructor(handler: ConsumerHandler, opts?: Partial<ConsumerOptions>) {
    const groupId = opts?.groupId || process.env.STREAM_CONSUMER_GROUP || 'consumer';
    this.opts = {
      groupId,
      maxRetries: Number(opts?.maxRetries ?? process.env.STREAM_CONSUMER_MAX_RETRIES ?? 8),
      baseBackoffMs: Number(opts?.baseBackoffMs ?? process.env.STREAM_CONSUMER_BASE_BACKOFF_MS ?? 200),
    ...
    for (let attempt = 1; attempt <= this.opts.maxRetries; attempt += 1) {
      try {
        ...
        if (attempt >= this.opts.maxRetries) {
          ...
          // Dead letter is counted; handler may also persist DLQ records.
          return;
        }
